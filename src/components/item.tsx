...
interface ItemProps {
    ...
    onUpdate: (id: number,data: {state?: boolean; color?: string}) => void;
    isdark: boolean;
}
...
function Item(props: ItemProps) {
    ...
    const { text } = props;
    const baseColor = props.color === "none" 
    ? (props.isdark ? "white" : "black") 
    : props.color;
    const [iscolorvisible, setIsColorVisible] = useState<boolean>(false);
    const handleCheckboxChange = (e: ChangeEvent<HTMLInputElement>) => {
        props.onUpdate(props.id, { state: e.target.checked });
    }
    const handleColorChange = (e: ChangeEvent<HTMLInputElement>) => {
        props.onUpdate(props.id, { color: e.target.value });
    }
    ...
    const checkedid = `checkbox-${props.id}`;
return <>
    <ul className="list-group" >
        <li className="list-group-item"style={{backgroundColor: props.isdark? "rgba(65, 63, 63, 1)": "white", color: props.isdark? "white": "black", borderColor: props.isdark? "rgba(128, 128, 128, 0.253)": "rgba(0, 0, 0, 0.253)"}}>
            ...
            <input className="form-check-input me-1" type="checkbox" id={checkedid} checked={props.state} onChange={handleCheckboxChange}/>
            <label className="form-check-label" htmlFor="firstCheckbox" style={{textDecoration: props.state == true? 'line-through': 'none',
            color: props.state == true? 'gray': baseColor
            }}> {text}</label>
            ...
            <div className="color">
                {iscolorvisible && (
                    <input type="color" value={props.color} onChange={handleColorChange} className="inputcolor"/>
                )}
                {iscolorvisible && (
